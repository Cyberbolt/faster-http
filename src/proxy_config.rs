...
impl ProxyConfig {
    ...
    pub fn from_url(proxy_url: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let host = url.host_str().ok_or("Missing proxy host")?.to_string();
    ...
    pub fn to_reqwest_proxy(&self) -> Result<Proxy, Box<dyn std::error::Error>> {
        let proxy = match self.proxy_type {
            ProxyType::Http | ProxyType::Https => Proxy::http(&self.url)?,
            ProxyType::Socks5 => {
                // reqwest内置支持SOCKS5
                // 注意：这需要reqwest的socks feature
                Proxy::all(&self.url).map_err(|e| {
                    format!(
                        "SOCKS5 proxy error (ensure reqwest socks feature is enabled): {}",
                        e
                    )
                })?
            }
    ...
    pub fn from_python_params(
        ...
        if let Some(proxies_dict) = proxies {
            for (key, value) in proxies_dict.iter() {
                ...
                let proxy_config = ProxyConfig::from_url(proxy_url).map_err(|e| {
                    RequestError::new_err(format!("Invalid proxy URL for '{}': {}", key_str, e))
                })?;
                ...
                if key_str.ends_with("://") {
                    ...
                    system
                        .scheme_proxies
                        .insert(key_str.to_string(), proxy_config);
...
mod tests {
    ...
    fn test_proxy_system_url_selection() {
        ...
        system
            .scheme_proxies
            .insert("http://".to_string(), http_proxy);
    ...
    fn test_no_proxy_patterns() {
        let system = ProxySystem {
            default_proxy: Some(ProxyConfig::from_url("http://proxy.com:8080").unwrap()),
