...
// File upload processing - temporarily disabled during hyper migration
pub fn build_multipart_form(
    _files_data: HashMap<String, PyObject>,
) -> PyResult<String> {
    Err(crate::error::RequestError::new_err(
        "Multipart form upload temporarily disabled during hyper migration"
    ))
}

// Disabled during hyper migration
fn _process_file_upload(
    _py: Python,
    _file_obj: &PyObject,
    _field_name: &str,
) -> PyResult<String> {
    // Temporarily disabled during hyper migration
    return Err(crate::error::RequestError::new_err(
        "File upload temporarily disabled during hyper migration"
    ));
