...
impl SslConfig {
    ...
    pub fn from_python_params(
        ...
        if let Some(cert_param) = cert {
            if let Ok(cert_tuple) = cert_param.downcast::<PyTuple>() {
                ...
                if tuple_len >= 2 {
                    let cert_file = cert_tuple.get_item(0)?
                        .downcast::<PyString>()?
                        .to_str()?;
                    let key_file = cert_tuple.get_item(1)?
                        .downcast::<PyString>()?
                        .to_str()?;
    ...
    pub fn load_ca_bundle_from_env(&mut self) {
        ...
        let env_vars = ["SSL_CERT_FILE", "SSL_CERT_DIR", "REQUESTS_CA_BUNDLE", "CURL_CA_BUNDLE"];
    ...
    pub fn validate(&self) -> PyResult<()> {
        ...
        if let Some(ref ca_file) = self.ca_cert_file {
            if !ca_file.exists() {
                return Err(crate::error::create_ssl_error(&format!(
                    "CA certificate file not found: {}", ca_file.display()
                )));
            }
        }
        ...
        if let Some(ref client_cert) = self.client_cert {
            if !client_cert.cert_file.exists() {
                return Err(crate::error::create_ssl_error(&format!(
                    "Client certificate file not found: {}", client_cert.cert_file.display()
                )));
            }
            if !client_cert.key_file.exists() {
                return Err(crate::error::create_ssl_error(&format!(
                    "Client private key file not found: {}", client_cert.key_file.display()
                )));
            }
        }
