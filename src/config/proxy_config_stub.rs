...
impl ProxySystem {
    ...
    pub fn from_python_params(
        ...
        if let Some(proxies_dict) = proxies {
            for item in proxies_dict.items() {
                ...
                system
                    .proxies
                    .insert(scheme_str.to_lowercase(), proxy_config);
            }
        }
    ...
    fn parse_proxy_param(proxy_param: &PyAny) -> PyResult<ProxyConfig> {
        ...
        if let Ok(proxy_str) = proxy_param.downcast::<PyString>() {
            ...
            let url_item = proxy_dict.get_item("url")?.ok_or_else(|| {
                pyo3::exceptions::PyKeyError::new_err("proxy dict must contain 'url' key")
            })?;
            ...
            Ok(ProxyConfig { url, auth, headers })
        } else {
