...
class TestHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _handle_delay(self):
        ...
        try:
            delay_seconds = float(self.path.split("/")[-1])
    ...
    def _handle_digest_auth(self):
        ...
        if len(parts) >= 5:
            ...
            # expected_password is not needed for simplified digest auth

            auth_header = self.headers.get("Authorization", "")

            if auth_header.startswith("Digest "):
                # For simplicity, just check if digest auth is attempted
                # Full digest auth implementation is complex
        ...
        try:
            ...
            for i in range(count):
                try:
                    ...
                    self.wfile.flush()
                    time.sleep(0.01)  # Reduce delay for faster tests
                except (BrokenPipeError, ConnectionResetError, ConnectionAbortedError):
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            if self.thread and self.thread.is_alive():
                self.thread.join(timeout=0.5)  # Reduced timeout
        except Exception:
