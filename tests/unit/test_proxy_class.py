...
class TestProxyClass:
    ...
    def test_proxy_interface_consistency_comparison(self):
        ...
        httpx_attrs = set(attr for attr in dir(httpx_proxy) if not attr.startswith('_'))
        faster_attrs = set(attr for attr in dir(faster_proxy) if not attr.startswith('_'))
        
        # faster_http should not have attributes that httpx doesn't have
        extra_attrs = faster_attrs - httpx_attrs
        assert len(extra_attrs) == 0, f"faster_http Proxy has extra attributes that httpx Proxy doesn't have: {extra_attrs}"
    ...
    def test_proxy_parameter_compatibility_comparison(self):
        ...
        for case in test_cases:
            ...
            try:
                httpx_proxy = httpx.Proxy(*args, **kwargs)
                httpx_accepts = True
            except Exception:
                httpx_accepts = False
            ...
            try:
                faster_proxy = faster_http.Proxy(*args, **kwargs)
                faster_accepts = True
            except Exception:
