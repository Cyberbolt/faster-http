...
class TestHooksSystemComprehensive:
    ...
    def test_hook_types_support_comparison(self):
        ...
        httpx_hook_types = {
            'request': [sample_request_hook],
            'response': [sample_response_hook]
        }
    ...
    def test_hook_system_integration_comparison(self):
        ...
        httpx_client = httpx.Client(
            auth=httpx_auth,
            event_hooks={'request': [integration_hook]},
            timeout=10.0
        )
        assert httpx_client is not None
        assert hasattr(httpx_client, 'event_hooks')
    ...
    def test_multiple_hooks_execution_order_comparison(self):
        ...
        httpx_client = httpx.Client(event_hooks={
            'request': [httpx_hook_1, httpx_hook_2]
        })
        assert httpx_client is not None
        ...
        faster_client = faster_http.Client(event_hooks={
            'request': [faster_hook_1, faster_hook_2]
        })
        assert faster_client is not None
        ...
        configs = [
            {
                ...
                'timeout': 30.0,
                'event_hooks': {'request': [config_hook]}
            },
            {
                'headers': {'User-Agent': 'test-client'},
                'verify': True,
                'event_hooks': {'response': [config_hook]}
            },
            {
