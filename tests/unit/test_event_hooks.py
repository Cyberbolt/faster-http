...
class TestEventHooksSystem:
    ...
    def test_hook_error_handling_comparison(self, stable_server):
        ...
        try:
            httpx_response = httpx_client.get(error_url)
        ...
        try:
            faster_response = faster_client.get(error_url)
            assert False, "Should have raised HTTPStatusError"
    ...
    def test_hook_inspection_only_comparison(self):
        ...
        httpx_client = httpx.Client(
            event_hooks={
                'request': [try_modify_request],
                'response': [try_modify_response]
            }
        )
...
class TestAsyncEventHooks:
    ...
    def test_sync_hooks_with_async_client_should_fail_comparison(self):
        ...
        try:
            httpx_client = httpx.AsyncClient(
                event_hooks={
                    'request': [sync_request_hook],
                    'response': [sync_response_hook]
                }
            )
