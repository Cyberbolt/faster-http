...
class TestCoreFunctionality:
    ...
    def test_async_client_creation_with_parameters_comparison(self):
        ...
        try:
            httpx_client = httpx.AsyncClient(
                base_url=base_url, timeout=timeout, headers=headers, params=params, http2=True
            )
            httpx_http2_works = True
        except Exception:
            # If httpx doesn't support http2, test without it
            httpx_client = httpx.AsyncClient(base_url=base_url, timeout=timeout, headers=headers, params=params)
        ...
        try:
            ...
            faster_client = faster_http.AsyncClient(base_url=base_url, timeout=timeout, headers=headers, params=params)
    ...
    def test_request_constructor_comparison(self):
        ...
        httpx_request = httpx.Request(method=method, url=url, headers=headers, content=content)
        ...
        faster_request = faster_http.Request(method=method, url=url, headers=headers, content=content)
    ...
    def test_timeout_object_comparison(self):
        ...
        faster_timeout = faster_http.Timeout(
            connect=connect_timeout, read=read_timeout, write=write_timeout, pool=pool_timeout
        )
    ...
    def test_headers_object_comparison(self):
        ...
        headers_data = {"Content-Type": "application/json", "User-Agent": "test-client/1.0", "Accept": "*/*"}
    ...
    def test_interface_consistency_comparison(self):
        ...
        httpx_client_attrs = {attr for attr in dir(httpx.Client) if not attr.startswith("_")}
        faster_client_attrs = {attr for attr in dir(faster_http.Client) if not attr.startswith("_")}
        ...
        assert len(extra_attrs) == 0, (
            f"faster_http.Client has extra attributes that httpx.Client doesn't have: {extra_attrs}"
        )

        # Test AsyncClient consistency
        httpx_async_attrs = {attr for attr in dir(httpx.AsyncClient) if not attr.startswith("_")}
        faster_async_attrs = {attr for attr in dir(faster_http.AsyncClient) if not attr.startswith("_")}

        extra_async_attrs = faster_async_attrs - httpx_async_attrs
        assert len(extra_async_attrs) == 0, (
            f"faster_http.AsyncClient has extra attributes that httpx.AsyncClient doesn't have: {extra_async_attrs}"
        )
        ...
        httpx_request_attrs = {attr for attr in dir(httpx_request) if not attr.startswith("_")}
        httpx_client.close()

        faster_request = faster_http.Request("GET", "https://example.com")
        faster_request_attrs = {attr for attr in dir(faster_request) if not attr.startswith("_")}

        extra_request_attrs = faster_request_attrs - httpx_request_attrs
        assert len(extra_request_attrs) == 0, (
            f"faster_http.Request has extra attributes that httpx.Request doesn't have: {extra_request_attrs}"
        )
