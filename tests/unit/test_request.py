...
class TestRequestConstructor:
    ...
    def test_request_with_empty_content_comparison(self):
        ...
        httpx_request = httpx.Request(method=method, url=url, content=empty_content)
        ...
        faster_request = faster_http.Request(method=method, url=url, content=empty_content)
    ...
    def test_request_with_none_content_comparison(self):
        ...
        try:
            httpx_request = httpx.Request(method=method, url=url, content=None)
        ...
        try:
            faster_request = faster_http.Request(method=method, url=url, content=None)
    ...
    def test_request_with_headers_object_comparison(self):
        ...
        httpx_request = httpx.Request(method=method, url=url, headers=httpx_headers)
        ...
        faster_request = faster_http.Request(method=method, url=url, headers=faster_headers)
    ...
    def test_request_properties_consistency_comparison(self):
        ...
        [attr for attr in dir(faster_request) if not attr.startswith("_")]
    ...
    def test_request_interface_consistency_comparison(self):
        ...
        httpx_attrs = {attr for attr in dir(httpx_request) if not attr.startswith("_")}
        faster_attrs = {attr for attr in dir(faster_request) if not attr.startswith("_")}
        ...
        assert len(extra_attrs) == 0, (
            f"faster_http Request has extra attributes that httpx Request doesn't have: {extra_attrs}"
        )
