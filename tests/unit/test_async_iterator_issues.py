...
class TestAsyncIteratorMethods:
    ...
    def test_iterator_interface_consistency_comparison(self, stable_server):
        ...
        httpx_iter_methods = set(attr for attr in dir(httpx_response) 
                                if not attr.startswith('_') and ('iter' in attr or 'aiter' in attr))
        faster_iter_methods = set(attr for attr in dir(faster_response) 
                                 if not attr.startswith('_') and ('iter' in attr or 'aiter' in attr))
        
        # faster_http should not have iterator methods that httpx doesn't have
        extra_methods = faster_iter_methods - httpx_iter_methods
        assert len(extra_methods) == 0, f"faster_http has extra iterator methods that httpx doesn't have: {extra_methods}"
