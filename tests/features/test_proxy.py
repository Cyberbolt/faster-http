...
class TestProxyConfiguration:
    ...
    def test_proxy_with_authentication_objects_comparison(self):
        ...
        httpx_client = httpx.Client(proxy=proxy_url, auth=httpx_auth)
        assert httpx_client is not None
        httpx_client.close()

        # Then test faster_http Client with same proxy and auth (should match httpx)
        faster_auth = faster_http.BasicAuth("user", "pass")
        faster_client = faster_http.Client(proxy=proxy_url, auth=faster_auth)
        assert faster_client is not None
        faster_client.close()
    ...
    def test_proxy_with_timeout_configuration_comparison(self):
        ...
        httpx_client = httpx.Client(proxy=proxy_url, timeout=timeout)
        assert httpx_client is not None
        httpx_client.close()

        # Then test faster_http Client with same proxy and timeout (should match httpx)
        faster_client = faster_http.Client(proxy=proxy_url, timeout=timeout)
        assert faster_client is not None
        faster_client.close()
    ...
    def test_proxy_with_headers_comparison(self):
        ...
        headers = {"User-Agent": "test-proxy-client/1.0", "X-Proxy-Test": "comparison"}

        # First test httpx Client with proxy and headers
        httpx_client = httpx.Client(proxy=proxy_url, headers=headers)
        assert httpx_client is not None
        ...
        faster_client = faster_http.Client(proxy=proxy_url, headers=headers)
        assert faster_client is not None
    ...
    def test_comprehensive_proxy_configuration_comparison(self):
        ...
        httpx_client = httpx.Client(proxy=proxy_url, headers=headers, timeout=timeout, verify=True, trust_env=True)
        ...
        faster_client = faster_http.Client(
            proxy=proxy_url, headers=headers, timeout=timeout, verify=True, trust_env=True
        )
