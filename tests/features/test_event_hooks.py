...
class TestEventHooksFeatures:
    ...
    def test_mixed_request_response_hooks_comparison(self, stable_server):
        ...
        httpx_client = httpx.Client(event_hooks={
        ...
        faster_client = faster_http.Client(event_hooks={
            'request': [faster_request_hook],
            'response': [faster_response_hook]
        })
    ...
    def test_async_event_hooks_comparison(self, stable_server):
        ...
        async def test_async_hooks():
            ...
            httpx_client = httpx.AsyncClient(event_hooks={
                'request': [httpx_async_request_hook],
                'response': [httpx_async_response_hook]
            })
            ...
            faster_client = faster_http.AsyncClient(event_hooks={
                'request': [faster_async_request_hook],
                'response': [faster_async_response_hook]
            })
    ...
    def test_hook_error_handling_comparison(self, stable_server):
        ...
        try:
            httpx_response = httpx_client.get(url)
            httpx_handled_error = False
        except Exception:
            httpx_handled_error = True
        finally:
            httpx_client.close()
        ...
        try:
            faster_response = faster_client.get(url)
            faster_handled_error = False
        except Exception:
            faster_handled_error = True
        finally:
            faster_client.close()
    ...
    def test_hooks_context_inspection_comparison(self, stable_server):
        ...
        httpx_client = httpx.Client(event_hooks={
            'request': [httpx_request_inspector],
            'response': [httpx_response_inspector]
        })
        httpx_response = httpx_client.post(url, json=data)
        assert httpx_response.status_code == 200
        ...
        faster_client = faster_http.Client(event_hooks={
            'request': [faster_request_inspector],
            'response': [faster_response_inspector]
        })
        faster_response = faster_client.post(url, json=data)
        assert faster_response.status_code == 200
