...
class TestIntegratedFeatures:
    ...
    def test_streaming_comprehensive_comparison(self, stable_server):
        ...
        with httpx.stream("GET", url) as httpx_stream:
            ...
            httpx_headers = httpx_stream.headers
        ...
        with faster_http.stream("GET", url) as faster_stream:
            ...
            faster_headers = faster_stream.headers
    ...
    def test_error_handling_comprehensive_comparison(self, stable_server):
        ...
        httpx_is_client_error = getattr(httpx_response, 'is_client_error', httpx_response.status_code >= 400 and httpx_response.status_code < 500)
        ...
        faster_is_client_error = getattr(faster_response, 'is_client_error', faster_response.status_code >= 400 and faster_response.status_code < 500)
        faster_is_success = getattr(faster_response, 'is_success', faster_response.status_code >= 200 and faster_response.status_code < 300)
    ...
    def test_interface_consistency_comprehensive_comparison(self, stable_server):
        ...
        httpx_attrs = set(attr for attr in dir(httpx_response) if not attr.startswith('_'))
        faster_attrs = set(attr for attr in dir(faster_response) if not attr.startswith('_'))
        
        # faster_http should not have attributes that httpx doesn't have
        extra_attrs = faster_attrs - httpx_attrs
        assert len(extra_attrs) == 0, f"faster_http Response has extra attributes that httpx Response doesn't have: {extra_attrs}"
        
        # Test Client interface consistency
        httpx_client = httpx.Client()
        faster_client = faster_http.Client()
        
        httpx_client_attrs = set(attr for attr in dir(httpx_client) if not attr.startswith('_'))
        faster_client_attrs = set(attr for attr in dir(faster_client) if not attr.startswith('_'))
        
        extra_client_attrs = faster_client_attrs - httpx_client_attrs
        assert len(extra_client_attrs) == 0, f"faster_http Client has extra attributes that httpx Client doesn't have: {extra_client_attrs}"
        
        httpx_client.close()
        faster_client.close()
        
        # Test module-level interface consistency
        httpx_module_attrs = set(attr for attr in dir(httpx) if not attr.startswith('_'))
        faster_module_attrs = set(attr for attr in dir(faster_http) if not attr.startswith('_'))
        
        # Check that essential httpx attributes exist in faster_http
        essential_attrs = {'Client', 'AsyncClient', 'Request', 'Response', 'get', 'post', 'put', 'patch', 'delete', 'head', 'options', 'BasicAuth'}
